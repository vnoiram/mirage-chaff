...
// (it was previously an unbounded sync.Map with CacheMax never wired in).
...
// The handler is built with a nil resolver, so any HEAD probe would fail and
// fall back to defaults — proving reuse by serving the cached content-type.
